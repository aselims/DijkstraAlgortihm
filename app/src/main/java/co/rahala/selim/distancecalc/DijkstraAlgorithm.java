...
public class DijkstraAlgorithm {
  ...
  private void findMinimalDistances(Vertex node) {
    ...
    for (Vertex neighbour : adjacentNodes) {
      if (getShortestDistance(neighbour) > getShortestDistance(node)
          + getDistance(node, neighbour)) {
        distance.put(neighbour, getShortestDistance(node)
            + getDistance(node, neighbour));
        neighbour.setMinDistance(distance.get(neighbour));
